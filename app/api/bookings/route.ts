...
async function sendEmailNotification(booking: any) {
  ...
  if (!gmailUser || !gmailAppPassword ||
      ...
      gmailAppPassword === "your_app_password_here") {
  ...
  const mailOptions = {
    from: `"Grand Auto Services" <${gmailUser}>`,
    to: receiverEmail,
...
export async function POST(request: Request) {
  try {
    ...
    // Await the email notification to ensure it completes before the serverless container is frozen
    try {
